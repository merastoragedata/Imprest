(function() {
  ...
  function renderAuth() {
    ...
    return '<div class="auth-wrap"><div class="auth-card">' +
      ...
      '<div class="field"><label>SAP ID</label><input id="au_user" placeholder="e.g. 25553" inputmode="numeric"></div>' +
  ...
  function renderForgotPassword() {
    ...
    var body;
    if (step === "ask") {
      body = '<div class="field"><label>SAP ID</label><input id="fp_user" placeholder="e.g. 25553" value="' + esc(state.forgotUser || "") + '"></div>' +
  ...
  function renderHome() {
    if (state.isAdmin) return renderAdminHome();
  ...
  function renderAdminHome() {
    ...
    return shell(body, "All Employees — PI &amp; TI", "Grouped by Financial Year (latest first), then by employee.");
  }
  ...
  var PRINT_CSS = "body{margin:0;font-family:Georgia,'Times New Roman',serif;color:#1a1a1a;-webkit-print-color-adjust:exact;print-color-adjust:exact}" + ".pg{width:210mm;min-height:297mm;box-sizing:border-box;padding:18mm 20mm;margin:0 auto 6mm;page-break-after:always}" + ".pg:last-child{page-break-after:auto}" + ".lt-org{font-weight:700;font-size:16px;text-align:center}" + ".lt-iso{text-align:center;font-size:11.5px;margin:5px 0 22px}" + ".lt-send{font-size:12px;margin-bottom:10px;white-space:pre-wrap}" + ".lt-ref{display:flex;justify-content:space-between;font-size:13px;margin-bottom:20px}" + ".lt-to{margin-bottom:16px}.lt-sub{margin-bottom:14px}.lt-salut{margin-bottom:10px}" + ".lt-para{margin-bottom:11px}" + ".lt-encl{margin-bottom:16px;font-size:12.5px}.lt-encl ol{margin:4px 0 0 18px;padding:0}" + ".lt-emp{text-align:right;margin:24px 0 28px}" + ".lt-rec{border-top:1px dashed #ccc;padding-top:14px}" +
    ".vch{border:1px solid #999;padding:12px 16px;font-size:11.5px;font-family:'Times New Roman',Georgia,serif;height:48%;box-sizing:border-box;overflow:hidden}" + ".vch-org{font-weight:700;font-size:12.5px;text-align:center;margin:2px 0 10px;font-family:Arial,sans-serif}" + ".vch-row{margin-bottom:5px;padding-bottom:2px;display:flex}" + ".vch-row b{flex:none;width:16px}" + ".vch-label{flex:none;width:118px}" + ".vch-cert-title{font-weight:700;margin:8px 0 5px}" + ".vch-sign{text-align:right;font-weight:700;margin-top:16px}" + ".vch-pair{display:flex;flex-direction:column;gap:4mm;height:100%}" + ".vch-divider{border-top:1px solid #ccc;margin:2mm 0}" +
    ...
    ".f2-particulars{border:1px dashed #999;padding:3px 6px;text-align:left;display:inline-block;width:100%;box-sizing:border-box}" +
  ...
  function form2PrintHtml(e) {
    ...
    var rowsHtml = d.rows.map(function(r) {
      var particulars = r.particulars ? '<span class="f2-particulars">' + esc(r.particulars) + "</span>" : "";
      return "<tr><td>" + r.sno + "</td><td>" + r.monthDate + "</td><td>" + esc(r.voucherNo) + "</td><td>" + particulars + '</td><td class="num">' + r.paymentRs + '</td><td class="num">' + r.totalRs + "</td><td></td></tr>";
    }).join("");
    var settleHtml = d.balance >= 0
      ? '<div class="f2-foot"><b>Amount paid back =</b> Rs. ' + plainNum(d.paidBack) + "</div>" +
        ...
        '<div class="f2-foot" style="margin-left:14px">Account No : ' + esc(d.refundAccount || "") + "</div>" +
        '<div class="f2-foot" style="margin-left:14px">Transaction No : UTR - ' + esc(d.refundUTR || "") + "</div>" +
      ...
      : '<div class="f2-foot"><b>Extra amount paid from pocket =</b> Rs. ' + plainNum(d.paidBack) + "</div>" +
        ...
        '<div class="f2-foot" style="margin-left:14px">Reimbursement UTR : ' + esc(d.refundUTR || "") + "</div>";
    return '<div class="pg f2-page">' +
      logoImgHtml(58) +
      '<div class="f2-title">FORM-2 IMPREST CASH ACCOUNT</div>' +
      '<table class="f2-head"><tr>' +
        '<td style="width:52%">Imprest Cash Book of :- ' + esc(d.name) + (d.designation ? ", " + esc(d.designation) : "") + '<br>SAP No-' + esc(d.sap) + '</td>' +
        '<td style="width:24%">CPF No:- ' + esc(d.cpf) + '</td>' +
        '<td style="width:24%">PMO No.' + (d.pmo ? " " + esc(d.pmo) : "") + '</td>' +
      "</tr><tr>" +
        '<td colspan="2">Date from : ' + fmtDate(d.dateFrom) + '</td>' +
      ...
      '<table class="f2-table"><thead><tr><th rowspan="2">Sr No</th><th rowspan="2">Month &amp; Date</th><th rowspan="2">Voucher No</th><th rowspan="2">Transactions</th><th colspan="2">Amount of Cash Payment</th><th colspan="2">Total</th><th rowspan="2">Head of<br>Account (SAP)</th></tr><tr><th>Rs</th><th>Ps</th><th>Rs</th><th>Ps</th></tr></thead><tbody>' +
        d.rows.map(function(r) {
          var particulars = r.particulars ? '<span class="f2-particulars">' + esc(r.particulars) + "</span>" : "";
          return "<tr><td>" + r.sno + "</td><td>" + r.monthDate + "</td><td>" + esc(r.voucherNo) + "</td><td style=\"text-align:left\">" + particulars + '</td><td class="num">' + r.paymentRs + '</td><td></td><td class="num">' + r.totalRs + "</td><td></td><td></td></tr>";
        }).join("") +
      "</tbody>" +
  ...
  function pavatiHtmlContent(e, tx) {
    ...
    return '<div class="pav-page"><div class="pav-title">पावती</div>' +
      ...
      '<div class="pav-line"><b>पावती लिहून घेणार :-</b> &nbsp;&nbsp;' + esc(p.designation || "सहाय्यक अभियंता") + ", " + esc(p.office || "400kV R.S Karjat") + "</div>" +
  ...
  function downloadBlankForm2Excel() {
    ensureXlsx().then(function() {
      var aoa = [ [ "FORM-2 IMPREST CASH ACCOUNT" ], [ "Imprest Cash Book of :- ______________________ , ______________________ SAP No-______________", "", "CPF No:- ______________", "", "PMO No. ______________" ], [ "Date from :", "______________", "", "Date to :", "______________" ], [] ];
      var headerRowIdx = aoa.length;
      aoa.push([ "Sr No", "Month & Date", "Voucher No", "Transactions", "Amount of Cash Payment (Rs)", "Total (Rs)", "Head of Account (SAP)" ]);
      for (var i = 0; i < 12; i++) aoa.push([ "", "", "", "", "", "", "" ]);
      var lastDataRow = aoa.length - 1;
      aoa.push([ "", "", "", "Total Rs. =", "", "", "" ]);
      aoa.push([]);
      aoa.push([ "Amount paid back =", "Rs. ______________" ]);
      aoa.push([ "Amount transferred details :" ]);
      aoa.push([ "Account No :", "" ]);
      aoa.push([ "Transaction No :", "UTR - ______________" ]);
      aoa.push([ "Date :", "" ]);
      aoa.push([]);
      aoa.push([]);
      aoa.push([ "", "", "", "", "", "______________________" ]);
      aoa.push([ "", "", "", "", "", "______________________" ]);
      aoa.push([ "", "", "", "", "", "______________________" ]);
      var ws = window.XLSX.utils.aoa_to_sheet(aoa);
      ws["!cols"] = [ { wch: 7 }, { wch: 13 }, { wch: 12 }, { wch: 36 }, { wch: 14 }, { wch: 12 }, { wch: 14 } ];
      try {
        for (var R = headerRowIdx; R <= lastDataRow; R++) {
          for (var C = 0; C <= 6; C++) {
            var addr = window.XLSX.utils.encode_cell({ r: R, c: C });
            if (!ws[addr]) ws[addr] = { t: "s", v: "" };
            ws[addr].s = { alignment: { horizontal: "center", vertical: "center", wrapText: true }, border: { top: { style: "thin" }, bottom: { style: "thin" }, left: { style: "thin" }, right: { style: "thin" } }, font: R === headerRowIdx ? { bold: true } : undefined };
          }
        }
      } catch (_e) {}
      var wb = window.XLSX.utils.book_new();
      window.XLSX.utils.book_append_sheet(wb, ws, "Form-2");
      window.XLSX.writeFile(wb, "Form2_Blank.xlsx", { cellStyles: true });
      toast("Excel downloaded.", "ok");
    }).catch(function() {
      toast("Could not load Excel library.", "err");
    });
  ...
  function downloadBlankVoucherExcel() {
    ensureXlsx().then(function() {
      var aoa = [
        ...
        [], [ "CERTIFICATE:" ],
        [ "1)", "Entry taken in L.P Register/Vehicle log book/MB for the purpose on Book No. …………… at page No. ………………….." ],
        [ "2)", "Paid from my pocket /Temp /Perm. Imprest & may be passed recipient." ],
        [], [ "", "", "", "Signature of Imprest Holder" ]
      ];
      var ws = window.XLSX.utils.aoa_to_sheet(aoa);
      ws["!cols"] = [ { wch: 5 }, { wch: 18 }, { wch: 3 }, { wch: 40 } ];
      var wb = window.XLSX.utils.book_new();
      window.XLSX.utils.book_append_sheet(wb, ws, "Voucher");
      window.XLSX.writeFile(wb, "Voucher_Blank.xlsx");
      toast("Excel downloaded.", "ok");
    }).catch(function() {
      toast("Could not load Excel library.", "err");
    });
  ...
  function renderFormats() {
    var card = function(title, desc, id) {
      return '<div class="card" style="margin-bottom:14px"><div class="card-head"><h3>' + title + "</h3></div>" +
        ...
        '<div class="btn-row">' +
          '<button class="btn btn-gold btn-sm" id="' + id + 'Excel">⬇ Excel (.xlsx)</button>' +
          '<button class="btn btn-ghost btn-sm" id="' + id + 'Pdf">⬇ PDF</button>' +
          '<button class="btn btn-ghost btn-sm" id="' + id + 'Word">⬇ Word (.docx)</button>' +
          '<button class="btn btn-ghost btn-sm" id="' + id + 'Print">🖶 Print</button>' +
        "</div></div>";
    };
    var body = '<div class="hint" style="margin-bottom:16px">Blank templates — same layout, logo, spacing and cell formatting as the filled documents everywhere else in the app.</div>' +
      card("Form-2 (Imprest Cash Account)", "Blank Form-2 register, ready to fill by hand or reuse as a template.", "fmtF2") +
      card("Voucher (2 per page)", "Blank imprest voucher, laid out two to a page to save paper.", "fmtVch") +
      card("Pavati", "Blank Marathi receipt (पावती).", "fmtPav");
    return shell(body, "Download Formats", "Blank Form-2, Voucher and Pavati templates in Excel and PDF.");
  }
  function wireFormats() {
    ...
    byId("fmtF2Word", function(el) { el.onclick = function() { toast("Use Excel or PDF for Form-2 — Word export isn't available for this one yet.", "err"); }; });
    ...
    byId("fmtVchWord", function(el) { el.onclick = downloadBlankVoucherWord; });
    ...
    byId("fmtPavExcel", function(el) { el.onclick = function() { toast("Pavati is a free-text receipt — use Word or PDF for a blank copy.", "err"); }; });
    byId("fmtPavPdf", function(el) { el.onclick = downloadBlankPavatiPdf; });
    byId("fmtPavWord", function(el) { el.onclick = downloadBlankPavatiWord; });
  }
