(function() {
  ...
  function renderViewEntry() {
    ...
    var docs = '<div class="card"><div class="card-head"><h3>Documents</h3></div>' +
      ...
      '<div class="btn-row" style="margin-top:8px"><button class="btn btn-ghost btn-sm" id="ve_vch_pdf">Vouchers PDF</button><button class="btn btn-ghost btn-sm" id="ve_vch_print">Vouchers Print (3/page)</button></div></div>';
  ...
  function accountsInner(e, t) {
    ...
    // Previous cycles as reverse-chronological collapsibles.
    var pastCyclesHtml = "";
    if (isMultiCycle) {
      ...
      pastCyclesHtml = '<div class="cat-head" style="margin-top:6px">Previous Cycles (latest first)</div>' + pastCyclesHtml;
    }
    ...
    var curCycleLabel = isMultiCycle ? '<div class="cat-head" style="margin-top:14px">Current Cycle (Cycle ' + curCycle.no + ")</div>" + recoupLine : "";
    var rows = renderLedger(e, curCycle);
    var curTotals = curCycle ? curCycle.totals : t;
    // Form-2 and Vouchers are collapsible subsections (default collapsed).
    var form2 = collapse("form2", "Form-2 (Imprest Cash Account)" + (isMultiCycle ? " — Cycle " + curCycle.no : ""), form2Inner(e, curTotals));
    var vouchers = collapse("vouchers", "Vouchers", vouchersInner(e, curCycle));
    ...
    var consolidated = isMultiCycle
      ...
      : "";
    var closureSection = '<div class="cat-head" style="margin-top:18px">Process ' + (e.type === "TI" ? "Imprest" : "PI") + " Closure</div>" + statusLine + actionButtons + '<div id="actionFormHost"></div>' + revertBtn;
    ...
    return banner + summary + pastCyclesHtml + curCycleLabel + makeEntryUI + '<div class="cat-head" style="margin-top:8px">Account Statement</div>' + (rows || '<div class="hint">No transactions yet.</div>') + balanceLine + form2 + vouchers + consolidated + closureSection + historySection;
  }
  ...
  function pastCycleInner(e, c, nextRecoup) {
    ...
    var f2btns = '<div class="btn-row" style="margin:6px 0">' + '<button class="btn btn-gold btn-xs" data-cycf2excel="' + c.no + '">⬇ Excel</button><button class="btn btn-ghost btn-xs" data-cycf2pdf="' + c.no + '">⬇ PDF</button><button class="btn btn-ghost btn-xs" data-cycf2print="' + c.no + '">🖶 Print</button></div>';
    var vchbtns = '<div class="btn-row" style="margin:6px 0"><button class="btn btn-ghost btn-xs" data-cycvchprint="' + c.no + '">🖶 Print (3/page)</button></div>';
    var closureInfo = "";
    if (c.recoup) {
      var r = c.recoup;
    ...
    return '<div class="cat-head" style="margin-top:4px">Account Statement — Cycle ' + c.no + "</div>" + (rows || '<div class="hint">No transactions.</div>') + bal + reimbLine +
      collapse("cycf2_" + c.no, "Form-2 — Cycle " + c.no, f2btns) +
      collapse("cycvch_" + c.no, "Vouchers — Cycle " + c.no, vchbtns) +
      closureInfo;
  }
  ...
  function downloadBlankVoucherExcel() {
    ...
    ensureExcelJS().then(function() {
      var wb = new window.ExcelJS.Workbook();
      var ws = wb.addWorksheet("Voucher");
      ws.columns = [ { width: 4 }, { width: 20 }, { width: 3 }, { width: 42 } ];
      var row = 1;
      var logo = logoImageBase64Raw();
      if (logo) {
        var imgId = wb.addImage({ base64: logo.base64, extension: logo.ext });
        ws.addImage(imgId, { tl: { col: 0, row: 0 }, ext: { width: 140, height: 52 } });
      ...
      org.alignment = { horizontal: "center" };
      row += 2;
      [
        [ "1)", "Voucher No", "______________" ],
        [ "2)", "Date", "______________" ],
        [ "3)", "Amount", "______________" ],
        [ "4)", "Name of Work", "______________________________" ],
        [ "5)", "Name of Agency", "______________________________" ]
      ].forEach(function(r) {
        ws.getCell(row, 1).value = r[0];
        ws.getCell(row, 1).font = { bold: true };
        ws.getCell(row, 2).value = r[1];
        ws.getCell(row, 4).value = r[2];
        row++;
      });
      row++;
      ...
      c1.alignment = { wrapText: true };
      row++;
      ws.mergeCells(row, 1, row, 4);
      ws.getCell(row, 1).value = "2)   Paid from my pocket /Temp /Perm. Imprest & may be passed recipient.";
      row += 2;
      ws.mergeCells(row, 1, row, 4);
      ...
      saveBlob(new Blob([buf], { type: "application/octet-stream" }), "Voucher_Blank.xlsx");
      toast("Excel downloaded.", "ok");
    }).catch(function(err) {
      toast("Could not build Excel: " + (err && err.message || "unknown error"), "err");
    });
