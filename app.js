(function() {
  ...
  function saveEntry(e) {
    ...
    apiPost("saveEntry", { username: state.user.username, entry: e }).then(function(res) {
  ...
  function profileComplete(p) {
    return p && p.name && p.designation && p.office && p.bossName && p.bossDesignation && p.bossOffice;
  }
  ...
  function renderAdminPanel() {
    ...
    function row(p, actions) {
      var methodBadge = p.authMethod === "google" ? '<span class="badge badge-info">Google · ' + esc(p.username) + "</span>" : '<span class="badge badge-neutral">Username · ' + esc(p.username) + "</span>";
    ...
    var approvedRows = approved.length ? approved.map(function(p) {
      return row(p, '<span class="badge badge-ok">Approved</span> <button class="btn btn-ghost btn-xs" data-revoke="' + p.username + '">Revoke</button>');
    }).join("") : '<div class="hint" style="padding:10px 2px">No approved accounts yet.</div>';
    return '<div class="topbar"><div class="topbar-row1"><div class="brand">' + boltSvg() + "<div><div>Imprest Portal · Admin</div></div></div>" + '<div class="topbar-spacer"></div><button class="user-pill" id="adminLogout"><span class="avatar">AD</span><span class="user-pill-name">Admin</span></button></div></div>' + '<div class="main" style="max-width:760px;margin:0 auto;padding:22px 20px">' + '<div class="page-head"><div><h1 class="page-title">Account approvals</h1><div class="page-sub">New logins (username or Google) must be approved here before employees can access the portal.</div></div></div>' + '<div class="card"><div class="card-head"><h3>Pending approval (' + pending.length + ")</h3></div>" + pendingRows + "</div>" + '<div class="card"><div class="card-head"><h3>Approved employees (' + approved.length + ")</h3></div>" + approvedRows + "</div>" + "</div>";
  }
  function wireAdminPanel() {
    byId("adminLogout", function(el) {
      el.onclick = function() {
        state.screen = "auth";
        state.authTab = "login";
        render();
  ...
  function renderAuth() {
    ...
    var isReg = mode === "register", isGoogle = mode === "google", isAdmin = mode === "admin";
    if (isAdmin) return renderAdminLogin();
    return '<div class="auth-wrap"><div class="auth-card">' + '<div class="auth-logo">' + boltSvg() + "<h1>Imprest Management Portal</h1><p>Temporary &amp; Permanent Imprest — request, track &amp; close</p></div>" + '<div class="tabs"><div class="tab ' + (mode === "login" ? "active" : "") + '" data-tab="login">Log in</div><div class="tab ' + (isReg ? "active" : "") + '" data-tab="register">Register</div><div class="tab ' + (isGoogle ? "active" : "") + '" data-tab="google">Google</div></div>' + (state.error ? '<div class="locked-note" style="background:var(--bad-100);color:var(--bad-500);margin-bottom:14px">⚠ ' + esc(state.error) + "</div>" : "") + (isGoogle ? '<div class="hint" style="margin-bottom:10px">Sign in with your Gmail. New accounts are linked to your Google Drive once approved — this prototype simulates the Google callback; real one-tap sign-in connects when we deploy on Apps Script.</div>' + '<div class="field"><label>Gmail address</label><input id="au_gmail" placeholder="you@gmail.com"></div>' + '<button class="btn btn-primary btn-block" id="googleSubmit">Continue with Google</button>' : '<div class="field"><label>Username</label><input id="au_user" placeholder="e.g. ashish.patil"></div>' + '<div class="field"><label>Password</label><input type="password" id="au_pass" placeholder="••••••••"></div>' + '<button class="btn btn-primary btn-block" id="authSubmit">' + (isReg ? "Create account" : "Log in") + "</button>" + (isReg ? '<div class="hint" style="margin-top:8px">New accounts need admin approval before first login. You will be able to log in once approved.</div>' : "")) + '<p class="hint" style="text-align:center;margin-top:16px"><a href="#" data-tab="admin">Admin login →</a></p>' + "</div></div>";
  }
  function renderAdminLogin() {
    return '<div class="auth-wrap"><div class="auth-card">' + '<div class="auth-logo">' + boltSvg() + "<h1>Admin Access</h1><p>Approve new employee accounts</p></div>" + (state.error ? '<div class="locked-note" style="background:var(--bad-100);color:var(--bad-500);margin-bottom:14px">⚠ ' + esc(state.error) + "</div>" : "") + '<div class="field"><label>Admin username</label><input id="ad_user" placeholder="admin"></div>' + '<div class="field"><label>Admin password</label><input type="password" id="ad_pass" placeholder="••••••••"></div>' + '<button class="btn btn-primary btn-block" id="adminSubmit">Log in as admin</button>' + '<p class="hint" style="text-align:center;margin-top:16px"><a href="#" data-tab="login">← Back to employee login</a></p>' + "</div></div>";
  }
  function shell(inner, title, sub, actions) {
    ...
    return '<div class="topbar"><div class="topbar-row1">' + '<button class="nav-toggle" id="sideToggle" title="Toggle menu">☰</button>' + '<div class="brand">' + boltSvg() + '<div><div>Imprest Portal</div><div class="sub">MSETCL · Petty Expense Imprest</div></div></div>' + '<div class="topbar-spacer"></div><div class="role-chip">Officer</div>' + '<button class="user-pill" id="logoutPill"><span class="avatar">' + esc(initials) + '</span><span class="user-pill-name">' + esc(u.name || u.username) + "</span></button>" + "</div></div>" + '<div class="shell"><nav class="sidenav' + (state.sideCollapsed ? " collapsed" : "") + '">' + navItem("home", "▤", "Dashboard") + navItem("profile", "◔", "My Profile") + navItem("otherexp", "⛁", "Other Expenses") + '<div class="nav-divider"></div>' + '<button class="nav-item" id="navCreate"><span class="ic">＋</span> <span class="lbl">Create new entry</span></button>' + '</nav><main class="main" id="main-content">' + '<div class="page-head"><div><h1 class="page-title">' + esc(title) + "</h1>" + (sub ? '<div class="page-sub">' + sub + "</div>" : "") + '</div><div class="page-actions">' + (actions || "") + "</div></div>" + inner + "</main></div>";
  }
  ...
  function accountsInner(e, t) {
    ...
    var summary = '<div class="grid grid-4" style="margin-bottom:14px">' + statCard("＋", "Received", "Rs. " + inr(t.recv)) + statCard("－", "Expense", "Rs. " + inr(t.exp)) + statCard("=", "Balance", "Rs. " + inr(t.balance)) + statCard("⚑", "Bills", t.noBill ? "Pending" : "All in") + "</div>";
    var processInitiated = e.type === "TI" ? !!e.closure : (e.recoupments || []).length > 0;
    var form2 = collapse("form2", "Generate Form-2 (Imprest Cash Account)", processInitiated ? form2Inner(e, t) : '<div class="locked-note">🔒 Available once you Process ' + (e.type === "TI" ? "Imprest Closure" : "Recoupment / PI Closure") + " below.</div>");
    var vouchers = collapse("vouchers", "Vouchers", processInitiated ? vouchersInner(e) : '<div class="locked-note">🔒 Available once you Process ' + (e.type === "TI" ? "Imprest Closure" : "Recoupment / PI Closure") + " below.</div>");
    ...
    var history = collapse("history", "Recoupment &amp; Closure History (Reimbursement / Pending Settlement)", historyInner(e));
    return banner + summary + (closed ? "" : '<button class="btn btn-ghost btn-sm" id="makeEntry" style="margin-bottom:12px">＋ Make entry in ' + e.type + " account</button>") + '<div class="cat-head" style="margin-top:4px">Account Statement</div>' + (rows || '<div class="hint">No transactions yet.</div>') + '<div id="txnFormHost"></div>' + actionButtons + '<div id="actionFormHost"></div>' + form2 + vouchers + history;
  }
  ...
  function form2Inner(e, t) {
    ...
    return fields + '<div class="stat-label" style="margin:4px 0 6px">Live Preview</div><div class="form2-scroll">' + table + "</div>" + '<div class="btn-row" style="margin-top:14px">' + '<button class="btn btn-gold btn-sm" id="f2_excel">⬇ Excel (.xlsx)</button>' + '<button class="btn btn-ghost btn-sm" id="f2_pdf">⬇ PDF</button>' + '<button class="btn btn-ghost btn-sm" id="f2_word">⬇ Word (open in Google Docs)</button>' + '<button class="btn btn-ghost btn-sm" id="f2_print">🖶 Print</button>' + "</div>" + '<div class="hint" style="margin-top:8px">Note: styled borders/center-alignment in the Excel export depend on the free client-side library\\u2019s support — if it doesn\\u2019t render, the Web preview, PDF, and Print all show the fully bordered/centered version.</div>';
  }
  ...
  function renderProfile() {
    var p = state.user;
    var methodNote = p.authMethod === "google" ? '<div class="locked-note" style="background:var(--info-100);color:var(--info-500);margin-bottom:14px">Signed in with Google (' + esc(p.username) + "). Google is used to verify your identity — once the Apps Script backend is connected, your documents will be created as real Google Docs/Sheets inside the organization\\u2019s shared Imprest Portal Drive folder (visible to Accounts and your reporting officer), not your personal Drive. For now, use Word/PDF downloads below.</div>" : '<div class="hint" style="margin-bottom:14px">Signed in with username &amp; password. You can download letters, vouchers and Form-2 as Word or PDF at any time.</div>';
    var body = '<div class="card" style="max-width:720px">' + methodNote + (!profileComplete(p) ? '<div class="locked-note" style="margin-bottom:14px">Complete your details before creating an entry.</div>' : "") + '<div class="card-head"><h3>Your details</h3></div>' + '<div class="field-row">' + f("pf_name", "Full name", p.name) + f("pf_desig", "Designation", p.designation) + "</div>" + f("pf_office", "Office", p.office) + f("pf_letter", "Standard office letter number", p.letterNo) + '<div class="field-row">' + f("pf_sap", "SAP No.", p.sapNo) + f("pf_cpf", "CPF No.", p.cpfNo) + f("pf_pmo", "PMO No.", p.pmoNo) + "</div>" + '<div class="card-head" style="margin-top:8px"><h3>Reporting officer</h3></div>' + f("pf_bname", "Boss's name", p.bossName) + '<div class="field-row">' + f("pf_bdesig", "Boss's designation", p.bossDesignation) + f("pf_boffice", "Boss's office", p.bossOffice) + "</div>" + f("pf_borg", "Boss's organisation", p.bossOrg) + '<div class="field"><label>Salutation</label><select id="pf_salut"><option value="Sir"' + (p.bossSalutation !== "Madam" ? " selected" : "") + '>Respected Sir,</option><option value="Madam"' + (p.bossSalutation === "Madam" ? " selected" : "") + ">Respected Madam,</option></select></div>" + '<button class="btn btn-primary btn-block" id="saveProfile">Save profile</button>' + "</div>" + '<div style="max-width:720px">' + collapse("pfletter", "Letter Preview", '<div id="pfLetterHost" class="letter-scale-outer">' + letterSheet(p, previewEntry(p)) + "</div>") + "</div>";
  ...
  function saveOtherExpense(exp) {
    ...
    apiPost("saveOtherExpense", { username: state.user.username, expense: exp }).then(function(res) {
      if (!res.ok) toast("Could not sync to server: " + (res.error || "unknown error"), "err");
    });
  }
  function deleteOtherExpense(id) {
    state.otherExpenses = state.otherExpenses.filter(function(x) { return x.id !== id; });
    apiPost("deleteOtherExpense", { username: state.user.username, id: id }).then(function(res) {
  ...
  function renderOtherExpenses() {
    ...
    var body = '<div class="card">' + '<button class="btn btn-gold btn-sm" id="addOtherExp" style="margin-bottom:14px">＋ Add expense</button>' + '<div id="otherExpFormHost"></div>' + rows + "</div>";
  ...
  function letterSheet(p, e) {
    ...
    return '<div class="letter-page"><div class="lt-org">MAHARASHTRA STATE ELECTRICITY TRANSMISSION COMPANY LTD.</div><div class="lt-iso">An ISO 9001:2000 CERTIFIED ORGANISATION</div>' + (L.sendingAddress ? '<div class="lt-send">' + esc(L.sendingAddress) + "</div>" : "") + '<div class="lt-ref"><span>' + esc(L.refPrefix || "Ref. [ Prefix ] No.") + " " + esc(L.refNumber || "__") + "</span><span>Date: " + fmtDate(L.refDate) + "</span></div>" + '<div class="lt-to">To,<br>The ' + esc(bd) + ",<br>" + esc(bo) + "<br>" + esc(bg) + "</div>" + '<div class="lt-sub"><b>Sub:</b>&nbsp;&nbsp;' + esc(e.subject || (e.type || "TI") + " Application of Rs [amount] for [purpose].") + "</div>" + '<div class="lt-salut">Respected ' + salut + ',</div><div class="lt-body">' + body + "</div>" + (atts.length ? '<div class="lt-encl"><b>Encl:</b><ol>' + atts.map(function(a) {
      return "<li>" + esc(a) + "</li>";
    }).join("") + "</ol></div>" : "") + '<div class="lt-emp"><div style="font-weight:700">' + esc(p.name || "[ Your name ]") + "</div><div>" + esc(p.designation || "[ Your designation ]") + "</div><div>" + esc(p.office || "[ Your office ]") + "</div></div>" + '<div class="lt-rec"><div style="font-weight:700">Recommended By,</div><div style="height:18px"></div><div style="font-weight:700">' + esc(p.bossName || "[ Boss's name ]") + "</div><div>" + esc(bd) + "</div><div>" + esc(bo) + "</div><div>" + esc(bg) + "</div></div></div>";
  }
  ...
  function render() {
    ...
    var html;
    if (state.screen === "auth") html = renderAuth(); else if (state.screen === "admin") html = renderAdminPanel(); else if (state.screen === "create") html = renderCreate(); else if (state.screen === "detail") html = renderDetail(); else if (state.screen === "profile") html = renderProfile(); else if (state.screen === "otherexp") html = renderOtherExpenses(); else html = renderHome();
  ...
  function wireAuth() {
    if (state.authTab === "admin") {
      ...
      byId("adminSubmit", function(el) {
        el.onclick = function() {
          doAdminLogin(val("ad_user"), val("ad_pass"));
        };
      });
      byId("ad_pass", function(el) {
        el.onkeydown = function(ev) {
          if (ev.key === "Enter") document.getElementById("adminSubmit").click();
        };
      });
      return;
    }
    ...
    if (state.authTab === "google") {
      byId("googleSubmit", function(el) {
        el.onclick = function() {
          doGoogleContinue(val("au_gmail"));
        };
      });
      byId("au_gmail", function(el) {
        el.onkeydown = function(ev) {
          if (ev.key === "Enter") document.getElementById("googleSubmit").click();
        };
      });
      return;
    }
  ...
  function delEntry(id) {
    ...
    apiPost("deleteEntry", { username: state.user.username, id: id }).then(function(res) {
      if (!res.ok) toast("Could not sync delete to server: " + (res.error || "unknown error"), "err");
    });
    toast("Deleted.", "ok");
    render();
  ...
  function letterPrintHtml(p, e) {
    ...
    return '<div class="pg"><div class="lt-org">MAHARASHTRA STATE ELECTRICITY TRANSMISSION COMPANY LTD.</div><div class="lt-iso">An ISO 9001:2000 CERTIFIED ORGANISATION</div>' + (L.sendingAddress ? '<div class="lt-send">' + esc(L.sendingAddress) + "</div>" : "") + '<div class="lt-ref"><span>' + esc(L.refPrefix || "") + " " + esc(L.refNumber || "") + "</span><span>Date: " + fmtDate(L.refDate) + "</span></div>" + '<div class="lt-to">To,<br>The ' + esc(bd) + ",<br>" + esc(bo) + "<br>" + esc(bg) + "</div>" + '<div class="lt-sub"><b>Sub:</b>&nbsp;&nbsp;' + esc(e.subject || "") + "</div>" + '<div class="lt-salut">Respected ' + salut + ",</div><div>" + body + "</div>" + (atts.length ? '<div class="lt-encl"><b>Encl:</b><ol>' + atts.map(function(a) {
      return "<li>" + esc(a) + "</li>";
    }).join("") + "</ol></div>" : "") + '<div class="lt-emp"><div style="font-weight:700">' + esc(p.name || "") + "</div><div>" + esc(p.designation || "") + "</div><div>" + esc(p.office || "") + "</div></div>" + '<div class="lt-rec"><div style="font-weight:700">Recommended By,</div><div style="height:18px"></div><div style="font-weight:700">' + esc(p.bossName || "") + "</div><div>" + esc(bd) + "</div><div>" + esc(bo) + "</div><div>" + esc(bg) + "</div></div></div>";
  }
  ...
  function printForm2(e) {
    ...
    var settleHtml = d.balance >= 0 ? "<div>Amount paid back = Rs. " + plainNum(d.paidBack) + '</div><div style="font-weight:700;margin-top:6px">Amount transferred details :</div>' + '<div style="margin-left:14px">Account No : ' + esc(d.refundAccount || "") + "</div>" + '<div style="margin-left:14px">Transaction No : UTR - ' + esc(d.refundUTR || "") + "</div>" + '<div style="margin-left:14px">Date : ' + (d.refundDate ? fmtDate(d.refundDate) : "") + "</div>" : "<div>Extra Amount Paid from Pocket = Rs. " + plainNum(d.paidBack) + '</div><div style="font-weight:700;margin-top:6px">Kindly reimburse the same.</div>' + '<div style="margin-left:14px">Reimbursement UTR : ' + esc(d.refundUTR || "") + "</div>";
    var html = '<div class="pg" style="font-family:Arial,sans-serif;font-size:12px">' + '<div style="text-align:center;font-weight:700;font-size:17px;margin-bottom:12px">FORM-2 IMPREST CASH ACCOUNT</div>' + "<div>Imprest Cash Book of :- " + esc(d.name) + " , " + esc(d.designation) + " SAP No-" + esc(d.sap) + "&nbsp;&nbsp;&nbsp;&nbsp;CPF No:- " + esc(d.cpf) + "&nbsp;&nbsp;&nbsp;&nbsp;PMO No. " + esc(d.pmo) + "</div>" + "<div>Date from : " + fmtDate(d.dateFrom) + "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Date to : " + fmtDate(d.dateTo) + "</div><br>" + '<table class="f2-table"><thead><tr><th>Sr No</th><th>Month &amp; Date</th><th>Voucher No</th><th>Transactions</th><th>Amount of<br>Cash Payment (Rs)</th><th>Total (Rs)</th><th>Head of<br>Account (SAP)</th></tr></thead><tbody>' + rowsHtml + "</tbody>" + '<tfoot><tr><td colspan="3" style="text-align:right;font-weight:700">Total Rs. =</td><td class="num" style="font-weight:700">' + d.totalPayment + '</td><td class="num" style="font-weight:700">' + d.totalReceipt + "</td><td></td></tr></tfoot></table><br>" + settleHtml + '<div style="text-align:right;font-weight:700;margin-top:22px">' + esc(d.name) + "<br>" + esc(d.designation) + "<br>" + esc(d.office) + "</div>" + "</div>";
  ...
  function formatParticulars(tx) {
    if (tx.kind === "received") {
      return tx.recoupmentId ? "Amount received as recoupment" : "Amount received as TI/PI";
    }
  ...
  function form2Data(e) {
    ...
    var rows = allTxns.map(function(tx, i) {
      ...
      return {
        sno: i + 1,
        monthDate: isRecv ? "" : fmtDate(tx.date),
        voucherNo: isRecv ? "" : displayVoucherNo(tx) || "",
        particulars: formatParticulars(tx),
  ...
  function buildForm2Excel(e) {
    var d = form2Data(e);
    var aoa = [ [ "FORM-2 IMPREST CASH ACCOUNT" ], [ "Imprest Cash Book of :- " + d.name + " , " + d.designation + " SAP No-" + d.sap, "", "CPF No:- " + d.cpf, "", "PMO No. " + d.pmo ], [ "Date from :", fmtDate(d.dateFrom), "", "Date to :", fmtDate(d.dateTo) ], [] ];
    ...
    aoa.push([]);
    if (d.balance >= 0) {
      aoa.push([ "Amount paid back =", "Rs. " + plainNum(d.paidBack) ]);
  ...
  function buildForm2Pdf(e) {
    ...
    doc.text("PMO No. " + d.pmo, W - M - 70, y, {
      align: "right"
    });
    y += 16;
    ...
    doc.setFontSize(10);
    if (d.balance >= 0) {
      doc.text("Amount paid back = Rs. " + plainNum(d.paidBack), M, y);
      y += 18;
      doc.setFont("helvetica", "bold");
  ...
  function buildForm2Word(e) {
    ...
    var children = [ new P({
      ...
      children: [ new T("Imprest Cash Book of :- " + d.name + " , " + d.designation + " SAP No-" + d.sap + "     CPF No:- " + d.cpf + "     PMO No. " + d.pmo) ]
    }), new P({
