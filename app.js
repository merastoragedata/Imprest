(function() {
  ...
  function cycleTabContent(e, cycle, isCurrent) {
    ...
    var summary = '<div class="grid grid-4" style="margin-bottom:14px">' +
      ...
      statCard("📋", "Vouchers", String(cycleExpenses.length)) + "</div>";
  ...
  function recoupmentTabContent(e) {
    ...
    if (!recoupItems.length) {
      ...
      histHtml = recoupItems.map(function(r) { return historyItemHtml({ kind: "recoup", data: r }); }).join("");
    }
  ...
  function piClosureTabContent(e) {
    ...
    var closureHtml = "";
    if (e.closure) {
      closureHtml = historyItemHtml({ kind: "closure", data: e.closure });
    } else {
    ...
    var finalHtml = finalRecoup.length ? finalRecoup.map(function(r) { return historyItemHtml({ kind: "recoup", data: r }); }).join("") : "";
    return processSection + '<div class="cat-head" style="margin-top:18px">Closure Details</div>' + closureHtml + finalHtml + revertBtn;
  }
  ...
  // TI layout — simple, no tabs, single cycle
  function accountsInnerTI(e, t) {
    ...
    if (!closed) {
      if (u.overspent) {
        banner = '<div class="util-banner util-bad">⚠ Amount exhausted — you are overspending on this TI (' + Math.round(u.pct) + "% of Rs. " + inr(u.limit) + "). Please process Imprest Closure.</div>";
      } else if (u.warn) {
    ...
    var statement = collapse("acctStatement", "Account Statement", (rows || '<div class="hint">No transactions yet.</div>') + balanceLine);
    var form2 = collapse("form2", "Form-2 (Imprest Cash Account)", form2Inner(e, t));
    var vouchers = collapse("vouchers", "Vouchers", vouchersInner(e, null));
    var sentToAccounts = e.status === "Sent to Accounts";
    ...
    var canRevertClosure = !!e.closure;
    var revertBtn = canRevertClosure ? '<button class="btn btn-ghost btn-sm" id="revertClosure" style="margin-bottom:12px">↺ Revert closure</button>' : "";
    var closureSection = collapse("closure", "Process Imprest Closure", statusLine + actionButtons + '<div id="actionFormHost"></div>' + revertBtn);
    var history = collapse("history", "Closure History", historyInner(e));
    return banner + summary + makeEntryUI + statement + form2 + vouchers + closureSection + history;
  }
  ...
  function consolidatedInner(e, t) {
    ...
    var bal = '<div class="balance-line" style="display:flex;justify-content:space-between;align-items:center;margin:10px 0 4px;padding:10px 14px;border:1px solid var(--line);border-radius:8px;background:var(--surface-2);font-weight:700"><span>Overall balance</span><span style="color:' + (t.balance < 0 ? "var(--bad-500)" : "var(--ok-500)") + '">Rs. ' + inr(t.balance) + (t.balance < 0 ? " (overspent)" : "") + "</span></div>";
    var totalsRow = '<div class="grid grid-3" style="margin:10px 0">' + statCard("＋", "Total Received", "Rs. " + inr(t.recv)) + statCard("－", "Total Expense", "Rs. " + inr(t.exp)) + statCard("=", "Balance", "Rs. " + inr(t.balance)) + "</div>";
    return '<div class="hint" style="margin-bottom:10px">Every transaction from all cycles, in order.</div>' + (rows || '<div class="hint">No transactions yet.</div>') + bal + totalsRow;
  }
  ...
  function vouchersInner(e, cycle) {
    var list = cycle ? cycle.txns.filter(function(t) { return t.kind === "expense"; }) : expenseTxns(e);
    return '<div class="hint" style="margin-bottom:10px">Every expense gets its own voucher (V1, V2…), printed three per page to save paper.</div>' + '<button class="btn btn-ghost btn-sm" id="previewVouchers" style="margin-bottom:10px">Preview all vouchers (' + list.length + ")</button>" + '<div id="voucherPreviewHost"></div>' + '<div class="btn-row" style="margin:10px 0"><button class="btn btn-gold btn-sm" id="allVouchersExcel">⬇ Excel</button><button class="btn btn-ghost btn-sm" id="allVouchersPdf">⬇ PDF</button><button class="btn btn-ghost btn-sm" id="allVouchersPrint">🖶 Print (3/page)</button></div>';
  }
  function wireVouchers(e, cycle) {
    byId("previewVouchers", function(el) {
      el.onclick = function() {
        ...
        host.innerHTML = '<div style="max-height:520px;overflow:auto;border:1px solid var(--line);border-radius:8px;padding:10px;background:var(--surface-2)">' + allVouchersPreviewHtml(e, cycle) + "</div>";
      };
    });
  ...
  function renderFormats() {
    ...
    return shell(body, "Download Formats", "Blank Form-2 and Pavati templates.");
  }
