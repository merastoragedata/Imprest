(function() {
  ...
  // NOTE ON STRUCTURE: every major sub-section below (Account Statement,
  // Form-2, Vouchers, each past Cycle, Consolidated, Closure, History) is
  // now its OWN independent top-level collapse() — none are nested inside
  // one another or inside a wrapping "accounts" collapse anymore. They
  // used to all be nested inside a single outer collapse("accounts", ...)
  // in renderDetail, which is the structure suspected of causing collapses
  // to not open/close reliably. Keeping every section flat also makes each
  // one easy to inspect independently.
  function accountsInner(e, t) {
    ...
    if (!closed) {
      if (u.overspent) {
        ...
        banner = '<div class="util-banner util-warn">⚠ ' + Math.round(u.pct) + "% of the amount received has been spent. Consider processing " + (e.type === "TI" ? "Imprest Closure" : "Recoupment") + ".</div>";
      }
    }
    ...
    var isMultiCycle = cycles && cycles.length > 1;
    var curCycle = cycles ? cycles[cycles.length - 1] : null;
    // Previous cycles — each its OWN top-level collapse, reverse-chronological.
    // A PI can go through any number of recoupment cycles before final
    // closure; this list grows by one entry every time a recoupment is
    // settled, with no upper bound.
    var pastCyclesHtml = "";
    if (isMultiCycle) {
      var past = cycles.slice(0, -1).reverse();
      pastCyclesHtml = past.map(function(c) {
        var nextRecoup = null;
        for (var ci = 0; ci < cycles.length; ci++) {
          if (cycles[ci].no === c.no + 1) { nextRecoup = cycles[ci].recoup; break; }
        }
        return collapse("cyc" + c.no, "Cycle " + c.no + " (Rs. " + inr(c.totals.recv) + " received · balance Rs. " + inr(c.totals.balance) + ")", pastCycleInner(e, c, nextRecoup));
      }).join("");
      pastCyclesHtml = '<div class="cat-head" style="margin-top:18px">Previous Cycles (' + past.length + ", latest first)</div>" + pastCyclesHtml;
    }
    // Disclosure line at the top of the current cycle summarising the recoupment
    // that opened it (only for cycle 2+).
    var recoupLine = "";
    if (curCycle && curCycle.recoup) {
      var rr = curCycle.recoup;
      recoupLine = '<div class="recoup-open-line" style="margin:8px 0 12px;padding:10px 14px;border:1px solid var(--gold-400);border-radius:8px;background:var(--surface-2);font-size:13px">' +
        "<b>Last recoupment:</b> Rs. " + inr(rr.totalCredit || 0) +
        " &nbsp;·&nbsp; <b>Adjusted in Cycle " + (curCycle.no - 1) + ":</b> Rs. " + inr(rr.overspentAmount || 0) +
        " &nbsp;·&nbsp; <b>Amount available for this cycle:</b> Rs. " + inr(rr.freshAmount || 0) + "</div>";
    }
    var rows = renderLedger(e, curCycle);
    var curTotals = curCycle ? curCycle.totals : t;
    var cycleTag = isMultiCycle ? " — Cycle " + curCycle.no + " (current)" : "";
    var balanceLine = '<div class="balance-line" style="display:flex;justify-content:space-between;align-items:center;margin:10px 0 4px;padding:10px 14px;border:1px solid var(--line);border-radius:8px;background:var(--surface-2);font-weight:700">' +
      "<span>Balance amount</span><span style=\"color:" + (curTotals.balance < 0 ? "var(--bad-500)" : "var(--ok-500)") + '">Rs. ' + inr(curTotals.balance) + (curTotals.balance < 0 ? " (overspent)" : "") + "</span></div>";
    var statement = collapse("acctStatement", "Account Statement" + cycleTag, recoupLine + (rows || '<div class="hint">No transactions yet.</div>') + balanceLine);
    // Form-2 and Vouchers are their own top-level collapsibles (default
    // collapsed), always scoped to the CURRENT cycle only — every past
    // cycle gets its own separate Form-2/Vouchers inside its own "Cycle N"
    // collapse above (see pastCycleInner), never mixed with this one.
    var form2 = collapse("form2", "Form-2 (Imprest Cash Account)" + cycleTag, form2Inner(e, curTotals));
    var vouchers = collapse("vouchers", "Vouchers" + cycleTag, vouchersInner(e, curCycle));
    var sentToAccounts = e.status === "Sent to Accounts";
    var statusLine = closed ? '<div class="badge badge-ok" style="margin-bottom:12px">This ' + e.type + " is closed.</div>" : sentToAccounts ? '<div class="badge badge-neutral" style="margin-bottom:12px">Sent to Accounts \u2014 awaiting final closure' + (e.closure && e.closure.remark ? ": " + esc(e.closure.remark) : "") + "</div>" : "";
    // Is there an unsettled recoupment for the current cycle? If so, the
    // "Process Recoupment" action is replaced with "Revert Recoupment".
    ...
    if (closed || sentToAccounts) {
      ...
      actionButtons = '<div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;margin-bottom:12px"><button class="btn btn-ghost btn-sm" id="revertRecoup" data-recoupid="' + pendingRecoup.id + '">↺ Revert Recoupment</button><span class="badge badge-ok">Recoupment processed — confirm settlement in history below</span></div>' +
        '<button class="btn btn-danger btn-block" id="processPIClose" style="font-size:15px;padding:14px">Process PI Closure (final)</button>';
    } else {
      actionButtons = '<div class="btn-row" style="margin-bottom:12px"><button class="btn btn-gold btn-block" id="processRecoup" style="font-size:15px;padding:14px">Process Recoupment</button><button class="btn btn-danger btn-block" id="processPIClose" style="font-size:15px;padding:14px">Process PI Closure (final)</button></div>';
    }
    ...
    var consolidated = isMultiCycle
      ? collapse("consolidated", "Consolidated Account — All Cycles (Received Rs. " + inr(t.recv) + " · Balance Rs. " + inr(t.balance) + ")", consolidatedInner(e, t))
      : "";
    var closureSection = collapse("closure", "Process " + (e.type === "TI" ? "Imprest" : "PI") + " Closure", statusLine + actionButtons + '<div id="actionFormHost"></div>' + revertBtn);
    var makeEntryUI = closed ? "" : '<button class="btn btn-gold btn-block" id="makeEntry" style="margin:4px 0 6px;font-size:15px;padding:14px">＋ Make entry in ' + e.type + ' account</button><div id="txnFormHost"></div>';
    return banner + summary + makeEntryUI + statement + form2 + vouchers + pastCyclesHtml + consolidated + closureSection + history;
  }
  ...
  // Read-only view of a settled past cycle: its statement, the reimbursement it
  // received (compensation from the recoupment that opened the NEXT cycle),
  // Form-2 downloads and closure record. `nextRecoup` is that next opener.
  function pastCycleInner(e, c, nextRecoup) {
    var rows = renderLedger(e, c);
    var bal = '<div class="balance-line" style="display:flex;justify-content:space-between;align-items:center;margin:10px 0 4px;padding:10px 14px;border:1px solid var(--line);border-radius:8px;background:var(--surface-2);font-weight:700"><span>Balance amount</span><span style="color:' + (c.totals.balance < 0 ? "var(--bad-500)" : "var(--ok-500)") + '">Rs. ' + inr(c.totals.balance) + (c.totals.balance < 0 ? " (overspent)" : "") + "</span></div>";
    var reimbLine = "";
    if (nextRecoup && (nextRecoup.overspentAmount || 0) > 0) {
      reimbLine = '<div class="hint" style="margin:2px 0 8px;padding:8px 14px;border-left:3px solid var(--ok-500);background:var(--surface-2)">Reimbursement received for this cycle: <b style="color:var(--ok-500)">Rs. ' + inr(nextRecoup.overspentAmount) + "</b> (compensation for overspend, credited with the next recoupment).</div>";
    }
    var f2btns = '<div class="cat-head" style="margin-top:14px;font-size:13px">Form-2 — Cycle ' + c.no + "</div>" + '<div class="btn-row" style="margin:6px 0">' + '<button class="btn btn-gold btn-xs" data-cycf2excel="' + c.no + '">⬇ Excel</button><button class="btn btn-ghost btn-xs" data-cycf2pdf="' + c.no + '">⬇ PDF</button><button class="btn btn-ghost btn-xs" data-cycf2print="' + c.no + '">🖶 Print</button></div>';
    var vchbtns = '<div class="cat-head" style="margin-top:10px;font-size:13px">Vouchers — Cycle ' + c.no + " (" + c.txns.filter(function(x) { return x.kind === "expense"; }).length + ")</div>" + '<div class="btn-row" style="margin:6px 0">' + '<button class="btn btn-gold btn-xs" data-cycvchexcel="' + c.no + '">⬇ Excel</button><button class="btn btn-ghost btn-xs" data-cycvchpdf="' + c.no + '">⬇ PDF</button><button class="btn btn-ghost btn-xs" data-cycvchprint="' + c.no + '">🖶 Print (3/page)</button></div>';
    var closureInfo = "";
    if (c.recoup) {
      var r = c.recoup;
      closureInfo = '<div class="closure-card" style="margin-top:8px"><b>Cycle opened by recoupment</b> on ' + fmtDate(r.date) + '<div class="hint">Adjusted for previous cycle Rs. ' + inr(r.overspentAmount || 0) + " · Opening balance Rs. " + inr(r.freshAmount || 0) + " · Total recoupment Rs. " + inr(r.totalCredit || 0) + "</div></div>";
    }
    // Form-2 and Vouchers here are plain button rows, NOT further nested
    // collapses — a past cycle's whole content is only ever one collapse
    // deep (this "Cycle N" collapse itself), never two-plus levels nested.
    return '<div class="cat-head" style="margin-top:4px">Account Statement — Cycle ' + c.no + "</div>" + (rows || '<div class="hint">No transactions.</div>') + bal + reimbLine +
      f2btns + vchbtns + closureInfo;
  }
  ...
  function wireDetail() {
    ...
    wireHistory(e);
    wireForm2(e);
    // Per-cycle download/print for past cycles (PI multi-cycle view).
    function cycleByNo(no) {
      var cy = cyclesOf(e);
      for (var i = 0; i < cy.length; i++) if (cy[i].no === no) return cy[i];
      return null;
    }
    var curCyc = e.type === "PI" ? cycleByNo(currentCycleNo(e)) : null;
    wireVouchers(e, curCyc);
    document.querySelectorAll("[data-cycf2excel]").forEach(function(el) {
      el.onclick = function() {
        var c = cycleByNo(parseInt(el.getAttribute("data-cycf2excel"), 10));
        if (!c) return;
        toast("Preparing Excel…", "ok");
        ensureExcelJS().then(function() {
          return buildForm2ExcelSheet(form2Data(e, c), "Form2_Cycle" + c.no + "_" + (e.letter.refNumber || e.id) + ".xlsx");
        }).then(function() { toast("Excel downloaded.", "ok"); }).catch(function(err) { toast("Could not build Excel: " + (err && err.message || "error"), "err"); });
      };
    });
    document.querySelectorAll("[data-cycf2pdf]").forEach(function(el) {
      el.onclick = function() {
        var c = cycleByNo(parseInt(el.getAttribute("data-cycf2pdf"), 10));
        if (c) downloadHtmlPdf(form2PrintHtml(e, c), "Form2_Cycle" + c.no + "_" + (e.letter.refNumber || e.id) + ".pdf", { widthPx: 794 });
      };
    });
    document.querySelectorAll("[data-cycf2print]").forEach(function(el) {
      el.onclick = function() {
        var c = cycleByNo(parseInt(el.getAttribute("data-cycf2print"), 10));
        if (c) openPrintWindow(form2PrintHtml(e, c));
      };
    });
    document.querySelectorAll("[data-cycvchexcel]").forEach(function(el) {
      el.onclick = function() {
        var c = cycleByNo(parseInt(el.getAttribute("data-cycvchexcel"), 10));
        if (c) downloadAllVouchersExcel(e, c);
      };
    });
    document.querySelectorAll("[data-cycvchpdf]").forEach(function(el) {
      el.onclick = function() {
        var c = cycleByNo(parseInt(el.getAttribute("data-cycvchpdf"), 10));
        if (c) downloadHtmlPdf(allVouchersPreviewHtml(e, c), "Vouchers_Cycle" + c.no + "_" + (e.letter.refNumber || e.id) + ".pdf", { widthPx: 794, multiPage: true });
      };
    });
    document.querySelectorAll("[data-cycvchprint]").forEach(function(el) {
      el.onclick = function() {
        var c = cycleByNo(parseInt(el.getAttribute("data-cycvchprint"), 10));
        if (c) openPrintWindow(allVouchersPreviewHtml(e, c));
      };
    });
  }
  ...
  function wireForm2(e) {
    byId("f2_save", function(el) {
      el.onclick = function() {
        var F;
        if (e.type === "PI") {
          var curNo = currentCycleNo(e);
          if (curNo === 1) { F = e.form2 = e.form2 || {}; }
          else { e.cycleData = e.cycleData || {}; F = e.cycleData[curNo] = e.cycleData[curNo] || {}; }
        } else {
          F = e.form2 = e.form2 || {};
        }
        F.sapNo = val("f2_sap");
  ...
  function downloadAllVouchersPdf(e) {
    var html = allVouchersPreviewHtml(e);
    downloadHtmlPdf(html, "Vouchers_All.pdf", { widthPx: 794, multiPage: true });
  }
