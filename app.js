(function() {
  ...
  function utilization(e) {
    ...
    // Utilisation is expense as a % of money actually RECEIVED, and the warning
    // only shows once real expenses exist and cross the threshold — never at 0%.
    var recv = t.recv || 0;
    var pct = recv ? t.exp / recv * 100 : 0;
    return {
      cycleExp: cycleExp,
      limit: limit,
      pct: pct,
      overspent: t.balance < 0,
      warn: t.exp > 0 && t.balance >= 0 && pct >= WARN_PCT.PI
    };
  }
  ...
  function accountsInner(e, t) {
    ...
    // PI with more than one cycle: show past cycles as collapsibles, current
    // cycle interactive below. Single-cycle PI and all TI: original flat layout.
    var cycles = e.type === "PI" ? cyclesOf(e) : null;
    var pastCyclesHtml = "";
    if (cycles && cycles.length > 1) {
      var past = cycles.slice(0, -1).reverse(); // reverse chronological
      pastCyclesHtml = past.map(function(c) {
        return collapse("cyc" + c.no, "Cycle " + c.no + " (Rs. " + inr(c.totals.recv) + " received · balance Rs. " + inr(c.totals.balance) + ")", pastCycleInner(e, c));
      }).join("");
      pastCyclesHtml = '<div class="cat-head" style="margin-top:6px">Previous Cycles</div>' + pastCyclesHtml;
    }
    var curCycleLabel = cycles && cycles.length > 1 ? '<div class="cat-head" style="margin-top:14px">Current Cycle (Cycle ' + cycles[cycles.length - 1].no + ")</div>" : "";
    var rows = renderLedger(e, cycles ? cycles[cycles.length - 1] : null);
    var curTotals = cycles ? cycles[cycles.length - 1].totals : t;
    var form2 = collapse("form2", "Generate Form-2 (Imprest Cash Account)" + (cycles && cycles.length > 1 ? " — Cycle " + cycles[cycles.length - 1].no : ""), form2Inner(e, curTotals));
    var vouchers = collapse("vouchers", "Vouchers", vouchersInner(e, cycles ? cycles[cycles.length - 1] : null));
    ...
    var actionButtons = closed || sentToAccounts ? "" : e.type === "TI" ? '<button class="btn btn-danger btn-block" id="processClose" style="margin-bottom:12px;font-size:15px;padding:14px">Process Imprest Closure</button>' : '<div class="btn-row" style="margin-bottom:12px"><button class="btn btn-gold btn-block" id="processRecoup" style="font-size:15px;padding:14px">Process Recoupment</button><button class="btn btn-danger btn-block" id="processPIClose" style="font-size:15px;padding:14px">Process PI Closure (final)</button></div>';
    var isMultiCycle = cycles && cycles.length > 1;
    var canRevert = !!e.closure || (e.recoupments || []).length > 0;
    var revertBtn = canRevert ? '<button class="btn btn-ghost btn-sm" id="revertClosure" style="margin-bottom:12px">\u21ba Undo / edit last recoupment or closure</button>' : "";
    var history = collapse("history", "Recoupment and Closure History (Reimbursement / Pending Settlement)", historyInner(e));
    // Consolidated view across all cycles (whole-entry ledger with cycle dividers).
  ...
  // Read-only view of a settled past cycle: its statement, Form-2 downloads and closure record.
  function pastCycleInner(e, c) {
    ...
    var f2btns = '<div class="btn-row" style="margin:10px 0"><button class="btn btn-gold btn-xs" data-cycf2excel="' + c.no + '">⬇ Form-2 Excel</button><button class="btn btn-ghost btn-xs" data-cycf2pdf="' + c.no + '">⬇ Form-2 PDF</button><button class="btn btn-ghost btn-xs" data-cycf2print="' + c.no + '">🖶 Form-2 Print</button><button class="btn btn-ghost btn-xs" data-cycvchprint="' + c.no + '">🖶 Vouchers (3/pg)</button></div>';
    var closureInfo = "";
    if (c.recoup) {
      var r = c.recoup;
      closureInfo = '<div class="closure-card" style="margin-top:8px"><b>Closed by recoupment</b> on ' + fmtDate(r.date) + '<div class="hint">Overspent settled Rs. ' + inr(r.overspentAmount || 0) + " · Fresh Rs. " + inr(r.freshAmount || 0) + " · Total credit Rs. " + inr(r.totalCredit || 0) + "</div></div>";
    }
    return '<div class="cat-head" style="margin-top:4px">Account Statement — Cycle ' + c.no + "</div>" + (rows || '<div class="hint">No transactions.</div>') + bal + f2btns + closureInfo;
  }
  ...
  function openClosureForm(e, mode) {
    ...
    if (mode === "close") {
      ...
      // The cycle is exhausted when the running balance (all money received,
      // including past recoupment credits, minus all expenses) has reached or
      // gone below zero. Using the balance directly avoids the earlier
      // array-position bug that under-counted current-cycle expenses.
      var exhausted = t.balance <= 0;
      if (!exhausted) {
        host.innerHTML = '<div class="closure-card"><div class="card-head"><h3>Process Recoupment</h3></div>' + '<div class="locked-note">⚠ Balance Amount Rs. ' + inr(t.balance) + " still available — the imprest isn't used up yet. Recoupment only applies once the balance reaches zero. Until then, only <b>Process PI Closure</b> can be processed.</div>" + '<button class="btn btn-ghost btn-sm" id="act_cancel" style="margin-top:10px">← Back</button></div>';
  ...
  function recoupFormHtml(overspent, fresh) {
    ...
    return '<div class="closure-card"><div class="card-head"><h3>Process Recoupment</h3></div>' +
      '<div class="hint" style="margin-bottom:10px">Previous cycle overspent by Rs. ' + inr(overspent) + ". Recoupment settles that and funds the next cycle.</div>" +
      ...
      '<div class="btn-row"><button class="btn btn-ghost btn-sm" id="rc_cancel">← Back</button><button class="btn btn-gold btn-sm" id="rc_save">Confirm recoupment &amp; start new cycle</button></div></div>';
  }
