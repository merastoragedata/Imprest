(function() {
  ...
  function utilization(e) {
    ...
    // Overspent/warn are driven by the real running balance, not the fragile
    // per-cycle expense figure, so the account banner matches the recoup gate.
    return {
      cycleExp: cycleExp,
      limit: limit,
      pct: limit ? cycleExp / limit * 100 : 0,
      overspent: t.balance < 0,
      warn: t.balance <= 0 || (limit ? cycleExp / limit * 100 >= WARN_PCT.PI : false)
    };
  }
  ...
  function renderAdminInner() {
    ...
    return userMgmt + adminData.accounts + allList;
  }
  ...
  function wireAdminPanel() {
    ...
    byId("ae_save", function(el) {
      el.onclick = function() {
        var p = state.adminEditUser;
        p.name = val("ae_name"); p.designation = val("ae_desig"); p.designationMr = val("ae_desigmr");
  ...
  // (latest first) then employee, plus the Accounts Section for pending closures.
  function renderAdminHome() {
    var all = state.allEntries || [];
    var pendingClosures = all.filter(function(e) { return e.status === "Sent to Accounts"; });
    var overspent = pendingClosures.filter(function(e) { return e.closure && e.closure.kind === "reimburse"; });
    var underspent = pendingClosures.filter(function(e) { return !e.closure || e.closure.kind !== "reimburse"; });
    function closureRow(e) {
      var c = e.closure || {};
      var owner = e._ownerName || e._owner || "Unknown";
      var isOver = c.kind === "reimburse";
      var acct = c.returnAcct;
      var detail = !isOver && acct ? '<div class="hint" style="margin-top:4px;color:var(--ok-500)">To A/c ' + esc(acct.accountNo || "") + (acct.ifsc ? " · " + esc(acct.ifsc) : "") + (acct.accountName ? " · " + esc(acct.accountName) : "") + (c.utr ? " · UTR " + esc(c.utr) : "") + "</div>" : "";
      return '<div class="closure-card" style="margin-bottom:10px"><div style="display:flex;justify-content:space-between;align-items:center;gap:8px"><b>' + esc(owner) + " \u2014 " + esc(e.type) + " \u00b7 Rs. " + inr(c.amount || 0) + "</b><span class=\"badge badge-neutral\">" + (isOver ? "Overspent" : "Underspent") + "</span></div>" +
        (c.remark ? '<div class="hint" style="margin-top:4px">' + esc(c.remark) + "</div>" : "") +
        detail +
        '<button class="btn btn-primary btn-xs" style="margin-top:8px" data-acctclose="' + e.id + '">' + (isOver ? "Mark Reimbursed to Employee" : "Mark Returned to Company\u2019s Account") + "</button></div>";
    }
    var accountsSection = '<div class="card" style="margin-bottom:20px"><div class="card-head"><h3>Accounts Section \u2014 pending closures (' + pendingClosures.length + ')</h3></div>' +
      (pendingClosures.length ? (overspent.length ? '<div class="hint" style="font-weight:700;margin:6px 0 4px">Overspent (' + overspent.length + ")</div>" + overspent.map(closureRow).join("") : "") +
        (underspent.length ? '<div class="hint" style="font-weight:700;margin:10px 0 4px">Underspent (' + underspent.length + ")</div>" + underspent.map(closureRow).join("") : "")
        : '<div class="hint">Nothing sent to Accounts yet.</div>') +
      "</div>";
    var byFY = {};
    ...
    return { accounts: accountsSection, list: body };
  }
  ...
  function accountsInner(e, t) {
    ...
    var closureSection = '<div class="cat-head" style="margin-top:18px">Imprest Closure</div>' + statusLine + actionButtons + '<div id="actionFormHost"></div>' + revertBtn + history;
    var balanceLine = '<div class="balance-line" style="display:flex;justify-content:space-between;align-items:center;margin:10px 0 4px;padding:10px 14px;border:1px solid var(--line);border-radius:8px;background:var(--surface-2);font-weight:700">' +
      "<span>Balance amount</span><span style=\"color:" + (t.balance < 0 ? "var(--bad-500)" : "var(--ok-500)") + '">Rs. ' + inr(t.balance) + (t.balance < 0 ? " (overspent)" : "") + "</span></div>";
    return banner + summary + (closed ? "" : '<button class="btn btn-ghost btn-sm" id="makeEntry" style="margin-bottom:12px">＋ Make entry in ' + e.type + " account</button>") + '<div class="cat-head" style="margin-top:4px">Account Statement</div>' + (rows || '<div class="hint">No transactions yet.</div>') + balanceLine + '<div id="txnFormHost"></div>' + form2 + vouchers + closureSection;
  }
  ...
  function wireProfile() {
    ...
    [ "pf_name", "pf_desig", "pf_desigmr", "pf_office", "pf_letter", "pf_bname", "pf_bdesig", "pf_boffice", "pf_borg" ].forEach(function(id) {
      byId(id, function(el) {
  ...
  function form2PrintHtml(e) {
    ...
    return '<div class="pg f2-page">' +
      ...
      '<table class="f2-head"><tr>' +
        '<td style="width:' + (d.pmo ? "52%" : "64%") + '">Imprest Cash Book of :- ' + esc(d.name) + (d.designation ? ", " + esc(d.designation) : "") + '<br>SAP No-' + esc(d.sap) + '</td>' +
  ...
  function buildPavatiWord(e, tx) {
    ...
    var children = [ new P({
      ...
      children: [ new T({
        ...
        bold: true
      }), new T((p.designationMr || "सहाय्यक अभियंता") + ", " + (p.office || "400kV R.S Karjat")) ]
    }), new P({
      text: ""
    }), new P({
      children: [ new T({
  ...
  function pavatiHtmlContent(e, tx) {
    ...
    return '<div class="pav-page"><div class="pav-title">पावती</div>' +
      ...
      '<div class="pav-line"><b>पावती लिहून घेणार :-</b> &nbsp;&nbsp;' + desigMr + ", " + esc(p.office || "400kV R.S Karjat") + "</div>" +
  ...
  function form2Data(e) {
    var p = docUser(), F = e.form2, t = totals(e);
  ...
  function renderFormats() {
    ...
    var body = '<div class="hint" style="margin-bottom:16px">Blank templates — same layout, logo, spacing and cell formatting as the filled documents everywhere else in the app.</div>' +
      ...
      card("Voucher (3 per page)", "Blank imprest voucher, laid out three to a page to save paper.",
        '<button class="btn btn-gold btn-sm" id="fmtVchExcel">⬇ Excel (.xlsx)</button><button class="btn btn-ghost btn-sm" id="fmtVchPdf">⬇ PDF</button><button class="btn btn-ghost btn-sm" id="fmtVchPrint">🖶 Print</button>') +
    ...
    return shell(body, "Download Formats", "Blank Form-2, Voucher and Pavati templates.");
  }
  function wireFormats() {
    ...
    byId("fmtVchPdf", function(el) { el.onclick = downloadBlankVoucherPdf; });
    byId("fmtVchPrint", function(el) { el.onclick = function() { openPrintWindow('<div class="pg"><div class="vch-pair">' + blankVoucherHtml() + '<div class="vch-divider"></div>' + blankVoucherHtml() + '<div class="vch-divider"></div>' + blankVoucherHtml() + "</div></div>"); }; });
