(function() {
  ...
  function doLogin(u, pw, remember) {
    ...
    apiPost("login", { username: u.trim(), password: pw }).then(function(res) {
      ...
      apiPost("adminLogin", { username: u.trim(), password: pw }).then(function(adminRes) {
        ...
        finishLogin({ username: u.trim(), name: "Administrator", designation: "Admin", isAdmin: true, refundAccounts: [] }, remember);
      });
    });
  }
  function finishLogin(profile, remember) {
    ...
    var after = function() {
      state.loading = false;
      state.screen = state.isAdmin || profileComplete(profile) ? "home" : "profile";
      render();
    };
    var afterAll = state.isAdmin
      ? loadAdminEmployees().then(loadAllData)
  ...
  function loadAllData() {
    ...
    return Promise.all([entriesCall, Promise.all(expCalls)]).then(function(res) {
      state.entries = res[0].sort(function(a, b) { return new Date(b.createdAt) - new Date(a.createdAt); });
  ...
  function renderAdminInner() {
    ...
    return createCard + '<div class="card"><div class="card-head"><h3>Pending approval (' + pending.length + ")</h3></div>" + pendingRows + "</div>" +
  ...
  function renderHome() {
    if (state.isAdmin && !state.adminOwnView) return renderAdminHome();
    var scope = state.scope;
    var base = state.isAdmin ? state.entries.filter(function(e) { return (e._owner || "") === state.user.username; }) : state.entries;
    ...
    var adminBack = state.isAdmin ? '<button class="btn btn-ghost btn-sm" id="adminBackAll">\u2190 All employees</button>' : "";
    var actions = toggle + adminBack + '<button class="btn btn-gold" id="createBtn2">＋ Create new entry</button>';
    return shell(stats + lists, state.isAdmin ? "My Imprests (Admin)" : "Dashboard", esc(state.user.office || ""), actions);
  }
  // Admin dashboard: every employee's PI and TI, grouped by Financial Year
  // (most recent first) and then by employee within each year.
  function renderAdminHome() {
    if (state.adminOwnView) return renderHome();
    var pendingClosures = state.entries.filter(function(e) { return e.status === "Sent to Accounts"; });
    ...
    function closureRow(e) {
      ...
      return '<div class="closure-card" style="margin-bottom:10px"><div style="display:flex;justify-content:space-between;align-items:center;gap:8px"><b>' + esc(owner) + " \u2014 " + esc(e.type) + " \u00b7 Rs. " + inr(c.amount || 0) + "</b><span class=\"badge badge-neutral\">" + (c.kind === "reimburse" ? "Overspent" : "Underspent") + "</span></div>" +
        (c.remark ? '<div class="hint" style="margin-top:4px">' + esc(c.remark) + "</div>" : "") +
        '<button class="btn btn-primary btn-xs" style="margin-top:8px" data-acctclose="' + e.id + '">Mark Closed by Accounts</button></div>';
    }
    ...
    var fyKeys = Object.keys(byFY).sort(function(a, b) {
      // "2025-26" style keys sort correctly as plain strings descending,
      // except the "—" bucket for entries with no date, which goes last.
      if (a === "—") return 1;
      if (b === "—") return -1;
    ...
    var body = fyKeys.length ? fyKeys.map(function(fy) {
      ...
      var ownerBlocks = owners.map(function(owner) {
        ...
        return '<div class="card" style="margin-bottom:12px"><div class="cat-head">' + esc(owner) + " — Rs. " + inr(list.reduce(function(s, e) { return s + (parseFloat(e.amount) || 0); }, 0)) + "</div>" +
          (pi.length ? '<div class="hint" style="margin:6px 0 4px;font-weight:700">PI</div>' + pi.map(function(e, i) { return entryCard(e, i + 1); }).join("") : "") +
          (ti.length ? '<div class="hint" style="margin:10px 0 4px;font-weight:700">TI</div>' + ti.map(function(e, i) { return entryCard(e, i + 1); }).join("") : "") +
          "</div>";
      }).join("");
    ...
    var toggle = '<div class="seg"><button class="on" data-adminscope="all">All employees</button><button data-adminscope="mine">My imprests</button></div>';
    return shell(accountsSection + body, "All Employees — PI &amp; TI", "Grouped by Financial Year (latest first), then by employee.", toggle);
  }
  ...
  function accountsInner(e, t) {
    ...
    return banner + summary + (closed ? "" : '<button class="btn btn-ghost btn-sm" id="makeEntry" style="margin-bottom:12px">＋ Make entry in ' + e.type + " account</button>") + '<div class="cat-head" style="margin-top:4px">Account Statement</div>' + (rows || '<div class="hint">No transactions yet.</div>') + '<div id="txnFormHost"></div>' + form2 + vouchers + closureSection;
  }
  ...
  function form2Inner(e, t) {
    var d = form2Data(e);
    var settleField = t.balance >= 0
      ? '<div class="field-row">' + f2v("f2_paidback", "Amount to be Paid Back (Rs.)", d.paidBack) + "</div>"
      : '<div class="field-row">' + f2v("f2_paidback", "Extra Amount Paid from Pocket (Rs.)", d.paidBack) + "</div>" + '<div class="locked-note" style="margin-bottom:12px">Kindly reimburse the same — bank details are captured in Imprest Closure below, not here.</div>';
  ...
  function wireHome() {
    ...
    document.querySelectorAll("[data-adminscope]").forEach(function(el) {
      el.onclick = function() {
        state.adminOwnView = el.getAttribute("data-adminscope") === "mine";
        render();
      };
    });
    byId("adminBackAll", function(el) {
      el.onclick = function() {
        state.adminOwnView = false;
        render();
      };
    ...
    document.querySelectorAll("[data-acctclose]").forEach(function(el) {
      el.onclick = function() {
        ...
        apiPost("saveEntry", { username: e._owner || ownerOf(e), entry: (function() { var p = Object.assign({}, e); delete p._owner; delete p._ownerName; return p; })() }).then(function(res) {
          if (!res.ok) { toast("Could not sync closure: " + (res.error || "unknown error"), "err"); return; }
          toast("Marked closed by Accounts.", "ok");
          render();
        });
      };
    });
  ...
  var PRINT_CSS = "@page{size:A4;margin:0}" + "body{margin:0;font-family:Georgia,'Times New Roman',serif;color:#1a1a1a;-webkit-print-color-adjust:exact;print-color-adjust:exact}" + ".pg{width:210mm;min-height:297mm;box-sizing:border-box;padding:18mm 20mm;margin:0 auto;page-break-after:always}" + ".pg:last-child{page-break-after:auto}" + ".lt-org{font-weight:700;font-size:16px;text-align:center}" + ".lt-iso{text-align:center;font-size:11.5px;margin:5px 0 22px}" + ".lt-send{font-size:12px;margin-bottom:10px;white-space:pre-wrap}" + ".lt-ref{display:flex;justify-content:space-between;font-size:13px;margin-bottom:20px}" + ".lt-to{margin-bottom:16px}.lt-sub{margin-bottom:14px}.lt-salut{margin-bottom:10px}" + ".lt-para{margin-bottom:11px}" + ".lt-encl{margin-bottom:16px;font-size:12.5px}.lt-encl ol{margin:4px 0 0 18px;padding:0}" + ".lt-emp{text-align:right;margin:24px 0 28px}" + ".lt-rec{border-top:1px dashed #ccc;padding-top:14px}" +
    ".vch{border:1px solid #999;padding:16px 20px;font-size:13px;font-family:'Times New Roman',Georgia,serif;height:48%;box-sizing:border-box;overflow:hidden}" + ".vch-org{font-weight:700;font-size:14px;text-align:center;margin:2px 0 14px;font-family:Arial,sans-serif}" + ".vch-row{margin-bottom:9px;padding-bottom:2px;display:flex}" + ".vch-row b{flex:none;width:18px}" + ".vch-label{flex:none;width:128px}" + ".vch-cert-title{font-weight:700;margin:12px 0 8px}" + ".vch-sign{text-align:right;font-weight:700;margin-top:22px}" + ".vch-pair{display:flex;flex-direction:column;gap:4mm;height:100%}" + ".vch-divider{border-top:1px solid #ccc;margin:2mm 0}" +
  ...
  function form2PrintHtml(e) {
    ...
    var settleHtml = d.balance >= 0
      ...
      : '<div class="f2-foot"><b>Extra amount paid from pocket =</b> Rs. ' + plainNum(d.paidBack) + "</div>" +
        '<div class="f2-foot" style="font-weight:700;margin-top:6px">Kindly reimburse the same \u2014 see Process Imprest Closure for UTR details.</div>';
  ...
  function pavatiHtmlContent(e, tx) {
    ...
    return '<div class="pav-page"><div class="pav-title">पावती</div>' +
      ...
      '<div class="pav-line"><b>पावती लिहून घेणार :-</b> &nbsp;&nbsp;' + esc(p.designationMr || "सहाय्यक अभियंता") + ", " + esc(p.office || "400kV R.S Karjat") + "</div>" +
  ...
  function allVouchersPreviewHtml(e) {
    ...
    for (var i = 0; i < list.length; i += 2) {
      ...
      pages += '<div class="pg"><div class="vch-pair">' + a + '<div class="vch-divider"></div>' + b + "</div></div>";
    }
    return pages;
  }
  ...
  function wireVouchers(e) {
    ...
    byId("blankVoucherPrint", function(el) {
      el.onclick = function() {
        openPrintWindow('<div class="pg"><div class="vch-pair">' + blankVoucherHtml() + '<div class="vch-divider"></div>' + blankVoucherHtml() + "</div></div>");
      };
    });
  }
  ...
  function buildForm2ExcelSheet(d, filename) {
    ...
    if (d.balance >= 0) {
      ...
      ws.getCell(row, 1).font = { bold: true };
      row++;
      ws.getCell(row, 1).value = "Kindly reimburse the same — see Process Imprest Closure for UTR details.";
      row += 2;
    }
  ...
  function renderFormats() {
    ...
    var body = '<div class="hint" style="margin-bottom:16px">Blank templates — same layout, logo, spacing and cell formatting as the filled documents everywhere else in the app.</div>' +
      ...
      card("Voucher (2 per page)", "Blank imprest voucher, laid out two to a page to save paper.",
  ...
  function wireFormats() {
    ...
    byId("fmtVchPrint", function(el) { el.onclick = function() { openPrintWindow('<div class="pg"><div class="vch-pair">' + blankVoucherHtml() + '<div class="vch-divider"></div>' + blankVoucherHtml() + "</div></div>"); }; });
